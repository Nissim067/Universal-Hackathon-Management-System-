import { db } from '../../config/db';
import { ApiError } from '../../utils/ApiError';
import { CreateHackathonInput, UpdateHackathonInput, GetHackathonsQuery } from './hackathon.schema';

export class HackathonService {
  async getHackathons(query: GetHackathonsQuery) {
    const { status, page, limit, search } = query;
    const skip = (page - 1) * limit;

    const where: any = {};
    if (status) {
      where.status = status;
    }
    if (search) {
      where.title = { contains: search, mode: 'insensitive' };
    }

    const [hackathons, total] = await Promise.all([
      db.hackathon.findMany({
        where,
        skip,
        take: limit,
        orderBy: { createdAt: 'desc' },
        include: {
          organizer: {
            select: { id: true, name: true, email: true },
          },
        },
      }),
      db.hackathon.count({ where }),
    ]);

    return {
      hackathons,
      pagination: {
        total,
        page,
        limit,
        totalPages: Math.ceil(total / limit),
      },
    };
  }

  async getHackathonById(id: string) {
    const hackathon = await db.hackathon.findUnique({
      where: { id },
      include: {
        organizer: {
          select: { id: true, name: true, email: true },
        },
        _count: {
          select: { registrations: true, teams: true, submissions: true },
        },
      },
    });

    if (!hackathon) {
      throw new ApiError(404, 'Hackathon not found', 'HACKATHON_NOT_FOUND');
    }

    return hackathon;
  }

  async createHackathon(data: CreateHackathonInput, organizerId: string) {
    const hackathon = await db.hackathon.create({
      data: {
        ...data,
        startDate: new Date(data.startDate),
        endDate: new Date(data.endDate),
        registrationDeadline: new Date(data.registrationDeadline),
        organizerId,
      },
    });
    return hackathon;
  }

  async updateHackathon(id: string, data: UpdateHackathonInput, userId: string) {
    const hackathon = await this.getHackathonById(id);

    if (hackathon.organizerId !== userId) {
      throw new ApiError(403, 'You are not authorized to update this hackathon', 'FORBIDDEN');
    }

    const updateData: any = { ...data };
    if (data.startDate) updateData.startDate = new Date(data.startDate);
    if (data.endDate) updateData.endDate = new Date(data.endDate);
    if (data.registrationDeadline) updateData.registrationDeadline = new Date(data.registrationDeadline);

    const updatedHackathon = await db.hackathon.update({
      where: { id },
      data: updateData,
    });

    return updatedHackathon;
  }

  async deleteHackathon(id: string, userId: string) {
    const hackathon = await this.getHackathonById(id);

    if (hackathon.organizerId !== userId) {
      throw new ApiError(403, 'You are not authorized to delete this hackathon', 'FORBIDDEN');
    }

    await db.hackathon.delete({
      where: { id },
    });

    return { message: 'Hackathon deleted successfully' };
  }

  async registerParticipant(hackathonId: string, userId: string) {
    const hackathon = await this.getHackathonById(hackathonId);

    if (hackathon.registrationFee > 0) {
      throw new ApiError(402, 'Payment required for this hackathon', 'PAYMENT_REQUIRED');
    }

    const now = new Date();
    if (now > hackathon.registrationDeadline) {
      throw new ApiError(400, 'Registration deadline has passed', 'REGISTRATION_CLOSED');
    }

    if (hackathon.maxParticipants) {
      const currentRegistrations = await db.registration.count({
        where: { hackathonId },
      });
      if (currentRegistrations >= hackathon.maxParticipants) {
        throw new ApiError(400, 'Hackathon is at full capacity', 'HACKATHON_FULL');
      }
    }

    const existingRegistration = await db.registration.findUnique({
      where: {
        userId_hackathonId: {
          userId,
          hackathonId,
        },
      },
    });

    if (existingRegistration) {
      throw new ApiError(409, 'You are already registered for this hackathon', 'DUPLICATE_REGISTRATION');
    }

    const registration = await db.registration.create({
      data: {
        userId,
        hackathonId,
      },
    });

    return registration;
  }
}
